...
class TubeTest {
    ...
    /**
     * Test method for {@link geometries.Tube#findGeoIntersectionsHelper(Ray, double)}.
     */
    @Test
    public void findGeoIntersectionsHelper() {
        ...
        Vector vAxis = new Vector(0, 0, 1);
        Tube tube2 = new Tube(new Ray(new Point(1, 1, 1), vAxis), 1d);
        Intersectable.GeoPoint p1 = new Intersectable.GeoPoint(tube1, new Point(1, 1, 1));
        Intersectable.GeoPoint p2 = new Intersectable.GeoPoint(tube1, new Point(1, 1, -1));
        Ray ray;
        //============ Equivalence Partitions Tests ==============//
        // EP1: Ray starts outside the tube and the distance is too small for both of the intersections point
        ray = new Ray(new Point(1, 1, 2), new Vector(0, 0, -1));
        assertNull(tube1.findGeoIntersectionsHelper(ray, 0.5), "Distance is too big");
        //EP2: Ray starts outside the tube and the distance is too small for one of the intersection points
        assertEquals(List.of(p1), tube1.findGeoIntersectionsHelper(ray, 1.5), "Distance is too big");
        //EP3: Enough distance for all intersections
        assertEquals(List.of(p1, p2), tube1.findGeoIntersectionsHelper(ray, 5), "Distance is too big");
        //EP4: Ray starts inside the tube
        ray = new Ray(new Point(1, 1, 0), new Vector(0, 0, -1));
        assertEquals(List.of(p2), tube1.findGeoIntersectionsHelper(ray, 5), "Distance is too big");
        //EP5: Ray starts inside the tube and distance is too short
        assertNull(tube1.findGeoIntersectionsHelper(ray, 0.35), "Distance is too big");

        // =============== Boundary Values Tests ==================
        //The distance equals to the t value, t1>0 t2>0
        //BVA1: Ray starts outside the tube and the distance is equal to t1
        ray = new Ray(new Point(1, 1, 2), new Vector(0, 0, -1));
        assertEquals(List.of(p1),tube1.findGeoIntersectionsHelper(ray, 1), "Distance is too big");
        //BVA2: Ray starts outside the tube and the distance is equal to t2
        assertEquals(List.of(p1,p2), tube1.findGeoIntersectionsHelper(ray, 3), "Distance is too big");

        //BVA4: Ray starts inside the tube and the distance is equal to t1
        ray = new Ray(new Point(1, 1, 0), new Vector(0, 0, -1));
        assertEquals(List.of(p2), tube1.findGeoIntersectionsHelper(ray, 1), "Distance is too big");
